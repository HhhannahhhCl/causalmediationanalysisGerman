...
def perform_intervention(intervention, model, effect_types=('indirect', 'direct')):
    ...
    candidate1 = ' '.join(intervention.candidates[0]).replace('Ġ', '') #Ġ needed?
    ...
    total_effect = (odds_alt - odds_base) / odds_base # p. 8: equivalent to odds_alt/odds_base - 1
...
def get_odds_ratio(intervention, model):
    # base_strings_tok is the list of lists of tokens for the two starting points (The doctor told the nurse that he/she)
    x = intervention.base_strings_tok[0]
    x_alt = intervention.base_strings_tok[1]
    with torch.no_grad():
        candidate1_base_prob, candidate2_base_prob = model.get_probabilities_for_examples_multitoken(
            x,
            intervention.candidates_tok) # candidates tok are the male and female continuation
        candidate1_alt_prob, candidate2_alt_prob = model.get_probabilities_for_examples_multitoken(
            x_alt,
            intervention.candidates_tok)
...
if __name__ == "__main__":
    ...
    interventions = [
        Intervention(
            tokenizer,
            "Der Arzt stellt dem Pfleger eine Frage. {}", #"The doctor asked the nurse a question. {}",
            ["Er", "Sie"], #["He", "She"],
            ["fragt", "antwortet"]),#["asked", "answered"]),
        Intervention(
            tokenizer,
            "Die Ärztin stellt der Pflegerin eine Frage. {}", #"The doctor asked the nurse a question. {}",
            ["Er", "Sie"], #["He", "She"],
            ["fragt", "antwortet"]) # ["requested", "responded"])
    ]
